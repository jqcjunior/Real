...
const App: React.FC = () => {
    ...
    const handleImportPerformanceData = async (data: any[]) => {
        try {
            for (const row of data) {
                await supabase.from('monthly_performance_actual').upsert({
            ...
            await fetchData();
        } catch (e) { alert("Erro ao salvar importação."); }
    };
        ...
        <div className="flex h-screen bg-gray-950 text-white overflow-hidden font-sans relative">
            ...
            <div className="flex-1 flex flex-col h-full bg-[#f3f4f6] overflow-hidden text-blue-950">
                ...
                <main className="flex-1 overflow-y-auto relative no-scrollbar">
                    {(() => {
                        if (currentView === 'dashboard_rede' && can('MODULE_DASHBOARD_ADMIN')) return <DashboardAdmin stores={stores} performanceData={performanceData} goalsData={goalsData} onImportPerformance={handleImportPerformanceData} onRefresh={fetchData} />;
