...
const App: React.FC = () => {
    ...
    const fetchData = async () => {
        try {
            ...
            if(s) setStores(s.map(x => ({
                id: x.id, number: x.number, name: x.name, city: x.city, state: x.state, status: x.status, has_gelateria: x.has_gelateria,
                managerName: x.manager_name, managerEmail: x.manager_email, managerPhone: x.manager_phone
            })));

            if(p) setPerformanceData(p.map(x => ({ 
                ...x, storeId: x.store_id, revenueTarget: Number(x.revenue_target || 0), revenueActual: Number(x.revenue_actual || 0), itemsActual: Number(x.items_actual || 0),
                salesActual: Number(x.sales_actual || 0), itemsPerTicket: Number(x.items_per_ticket || 0), unitPriceAverage: Number(x.unit_price_average || 0), 
                averageTicket: Number(x.average_ticket || 0), percentMeta: Number(x.percent_meta || 0), paTarget: Number(x.pa_target || 0), puTarget: Number(x.pu_target || 0), ticketTarget: Number(x.ticket_target || 0)
            })));
            ...
            if(ag) setAgenda(ag.map(x => ({
                ...x, userId: x.user_id, title: x.title, description: x.description, dueDate: x.due_date ? x.due_date.split('T')[0] : '', dueTime: x.due_time, priority: x.priority, isCompleted: x.is_completed
            })));
    ...
    const isIceCreamRole = user.role === UserRole.ICE_CREAM;
            ...
            <aside className={`fixed inset-y-0 left-0 z-[100] w-72 bg-gray-950 border-r border-white/5 flex flex-col p-6 transition-transform duration-300 transform ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'} shrink-0 overflow-y-auto no-scrollbar shadow-2xl`}>
                ...
                <nav className="flex-1 space-y-6">
                    {[
                        ...
                        { title: 'Operacional', items: [ { id: 'pdv_gelateria', label: 'PDV Gelateria', icon: IceCreamIcon, perm: 'MODULE_ICECREAM', requiredFeature: hasGelateria || isIceCreamRole }, { id: 'caixa', label: 'Caixa', icon: ClipboardList, perm: 'MODULE_CASH_REGISTER' }, { id: 'agenda', label: 'Agenda Semanal', icon: Calendar, perm: 'MODULE_AGENDA' } ] },
                ...
                <main className="flex-1 overflow-y-auto relative no-scrollbar">
                    {(() => {
                        ...
                        if (currentView === 'settings' && can('MODULE_SETTINGS')) return <AdminSettings stores={stores} onAddStore={async (s) => { 
                            const payload = {
                                number: s.number, name: s.name, city: s.city, state: s.state, status: s.status || 'active', has_gelateria: s.has_gelateria || false,
                                manager_name: s.managerName || '', manager_email: s.managerEmail || '', manager_phone: s.managerPhone || ''
                            };
                            ...
                            const payload = {
                                number: s.number, name: s.name, city: s.city, state: s.state, status: s.status, has_gelateria: s.has_gelateria,
                                manager_name: s.managerName || '', manager_email: s.managerEmail || '', manager_phone: s.managerPhone || ''
                            };
                        ...
                        if (isIceCreamRole) return <IceCreamModule user={user!} stores={stores} items={iceCreamItems} sales={iceCreamSales} finances={iceCreamFinances} stock={iceCreamStock} promissories={icPromissories} can={can} onAddSales={async (s) => { await supabase.from('ice_cream_daily_sales').insert(s.map(x => ({ store_id: x.storeId, item_id: x.itemId, product_name: x.productName, category: x.category, flavor: x.flavor, units_sold: Math.round(x.unitsSold), unit_price: x.unitPrice, total_value: x.totalValue, payment_method: x.paymentMethod, sale_code: x.saleCode, buyer_name: x.buyer_name, status: 'active' }))); fetchData(); }} onCancelSale={async (code, reason) => { await supabase.from('ice_cream_daily_sales').update({ status: 'canceled', cancel_reason: reason }).eq('sale_code', code); await fetchData(); }} onUpdatePrice={async (id, p) => { await supabase.from('ice_cream_items').update({ price: p }).eq('id', id); await fetchData(); }} onAddTransaction={async (t) => { await supabase.from('ice_cream_finances').insert([{ store_id: t.storeId, date: t.date, type: t.type, category: t.category, value: t.value, description: t.description }]); await fetchData(); }} onAddItem={async (n, c, p, f, si, u, cps, tsId, r) => { await handleSaveIceCreamProduct({ storeId: tsId, name: n, category: c as any, price: p, flavor: f, recipe: r }); }} onSaveProduct={handleSaveIceCreamProduct} onDeleteItem={async (id) => { await supabase.from('ice_cream_items').delete().eq('id', id); await fetchData(); }} onUpdateStock={async (sId, b, v, u, type) => { await supabase.from('ice_cream_stock').upsert({ store_id: sId, product_base: b, stock_current: v, unit: u }); fetchData(); }} liquidatePromissory={async (id) => { await supabase.from('ice_cream_promissory_notes').update({ status: 'paid' }).eq('id', id); await fetchData(); }} onDeleteStockItem={async (id) => { await supabase.from('ice_cream_stock').delete().eq('id', id); await fetchData(); }} />;
