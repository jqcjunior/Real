...
import { User, Store, MonthlyPerformance, UserRole, Cota, AgendaItem, DownloadItem, SystemLog, CashError, CotaSettings, CotaDebts, IceCreamItem, IceCreamDailySale, IceCreamTransaction, PagePermission, CashRegisterClosure, ProductPerformance, Receipt, IceCreamStock, IceCreamPromissoryNote } from './types';
...
const App: React.FC = () => {
  ...
  const loadModuleData = async () => {
    ...
    if (perf) setPerformanceData(perf.map(p => ({
        id: p.id, storeId: p.store_id, month: p.month, revenueTarget: Number(p.revenue_target || 0), revenueActual: Number(p.revenue_actual || 0), percentMeta: Number(p.percent_meta || 0), itemsTarget: Number(p.items_target || 0), itemsActual: Number(p.items_actual || 0), itemsPerTicket: Number(p.pa_actual || 0), unitPriceAverage: Number(p.pu_actual || 0), averageTicket: Number(p.averageTicket || 0), delinquencyRate: Number(p.delinquencyRate || 0), paTarget: Number(p.pa_target || 0), ticketTarget: Number(p.ticket_target || 0), puTarget: Number(p.pu_target || 0), delinquencyTarget: Number(p.delinquency_target || 0), businessDays: Number(p.business_days || 26), trend: p.trend || 'stable', correctedDailyGoal: Number(p.corrected_daily_goal || 0)
    })));
    ...
    if (icSa) setIceCreamSales(icSa.map(s => ({ 
        id: s.id, storeId: s.store_id, itemId: s.item_id, productName: s.product_name, category: s.category, flavor: s.flavor, unitsSold: s.units_sold, unitPrice: Number(s.unit_price), totalValue: Number(s.total_value), paymentMethod: s.payment_method as any, createdAt: s.created_at, saleCode: s.sale_code, status: s.status, buyer_name: s.buyer_name 
    })));
    ...
    if (ag) setAgendaTasks(ag.map(t => ({ 
        id: t.id, userId: t.user_id, title: t.title, description: t.description, dueDate: t.due_date, dueTime: t.due_time || '00:00:00', priority: t.priority, isCompleted: t.is_completed, createdAt: new Date(t.created_at), reminder_level: t.reminder_level || 1, reminded_at: t.reminded_at, completed_note: t.completed_note 
    })));
  };
  ...
  useEffect(() => {
    if (user?.role === UserRole.ADMIN) {
        setCurrentView('dashboard_admin');
    } else if (user?.role === UserRole.ICE_CREAM) {
        setCurrentView('icecream');
    } else {
        setCurrentView('dashboard_manager');
    }
  }, [user]);
  ...
  const handleIceCreamSale = async (salesList: IceCreamDailySale[]) => {
      for (const sale of salesList) {
          ...
          if (item && item.consumptionPerSale > 0) {
              const { data: stockItem } = await supabase.from('ice_cream_stock').select('*').eq('store_id', sale.storeId).limit(1).single();
              if (stockItem) {
                  await supabase.from('ice_cream_stock').update({
                      stock_current: stockItem.stock_current - (item.consumptionPerSale * sale.unitsSold)
                  }).eq('id', stockItem.id);
              }
          }
      }
    ...
    <div className="flex h-screen bg-gray-900 text-white overflow-hidden font-sans relative">
      <div className={`fixed lg:relative z-50 w-64 h-full bg-gray-950 border-r border-white/5 flex flex-col shadow-2xl transition-transform duration-300 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}>
        <div className="p-8 border-b border-white/5 mb-4 flex justify-between items-center">
          <div>
            ...
            <p className="text-[8px] font-black text-blue-400 uppercase tracking-widest mt-2 opacity-50">Enterprise v27.6</p>
          </div>
        ...
        <div className="flex-1 overflow-y-auto no-scrollbar space-y-6 px-2 pb-10">
          ...
          <div>
            ...
            <NavButton 
              ...
              onClick={() => { setCurrentView('access_control'); setIsSidebarOpen(false); }} 
              permission="MODULE_ACCESS_CONTROL" 
              can={can} 
            />
            <NavButton 
              view="audit" 
              ...
              onClick={() => { setCurrentView('audit'); setIsSidebarOpen(false); }} 
              permission="MODULE_AUDIT" 
              can={can} 
            />
            <NavButton 
              view="settings" 
        ...
        <div className="p-4 bg-black/40 border-t border-white/5">
            <button onClick={() => setUser(null)} className="w-full py-3 bg-white/5 hover:bg-red-900/20 text-gray-400 hover:text-red-400 rounded-xl text-[10px] font-black uppercase transition-all flex items-center justify-center gap-2">
      ...
      <div className="flex-1 flex flex-col h-full bg-[#f8fafc] overflow-hidden text-blue-950 relative">
        ...
        <main className="flex-1 overflow-y-auto no-scrollbar relative">
            {(() => {
                if (currentView === 'dashboard_admin' && can('MODULE_DASHBOARD_ADMIN')) return <DashboardAdmin stores={stores} performanceData={performanceData} onImportData={async (d) => { await supabase.from('monthly_performance').upsert(d, { onConflict: 'store_id,month' }); fetchData(); }} />;
                ...
                if (currentView === 'metas' && can('MODULE_METAS')) return <GoalRegistration stores={stores} performanceData={performanceData} onUpdateData={async (d) => { await supabase.from('monthly_performance').upsert(d, { onConflict: 'store_id,month' }); fetchData(); }} />;
                ...
                if (currentView === 'icecream' && can('MODULE_ICECREAM')) return <IceCreamModule user={user} stores={stores} items={iceCreamItems} stock={iceCreamStock} sales={iceCreamSales} finances={iceCreamFinances} promissories={iceCreamPromissories} can={can} onAddSales={handleIceCreamSale} onAddTransaction={async (tx) => { await supabase.from('ice_cream_finances').insert([tx]); fetchData(); }} onDeleteItem={async (id) => { await supabase.from('ice_cream_items').delete().eq('id', id); fetchData(); }} onAddItem={async (name, cat, price, flavor, initialStock, unit, cons, target) => { await supabase.from('ice_cream_items').insert([{ name, category: cat, price, flavor, consumption_per_sale: cons, store_id: target || user.storeId }]); fetchData(); }} onUpdatePrice={async (id, p) => { await supabase.from('ice_cream_items').update({ price: p }).eq('id', id); fetchData(); }} onCancelSale={async (code, reason) => { await supabase.from('ice_cream_daily_sales').update({ status: 'canceled', cancel_reason: reason, canceled_by: user.name }).eq('sale_code', code); fetchData(); }} onUpdateStock={async (sid, base, val, unit, type) => { const existing = iceCreamStock.find(s => s.store_id === sid && s.product_base === base); if (existing) await supabase.from('ice_cream_stock').update({ stock_current: type === 'production' ? existing.stock_current + val : val }).eq('id', existing.id); else await supabase.from('ice_cream_stock').insert([{ store_id: sid, product_base: base, stock_initial: val, stock_current: val, unit }]); fetchData(); }} liquidatePromissory={async (id) => { await supabase.from('ice_cream_promissory_notes').update({ status: 'paid' }).eq('id', id); fetchData(); }} />;
