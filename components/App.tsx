...
const App: React.FC = () => {
    ...
    const fetchPermissions = async (role: string) => {
        if (role === UserRole.ADMIN) { setUserPermissions([]); return; }
        let columnToCheck = '';
        const roleUpper = role.toUpperCase().trim();
        if (roleUpper === 'GERENTE') columnToCheck = 'allow_manager';
        else if (roleUpper === 'CAIXA') columnToCheck = 'allow_cashier';
        else if (roleUpper === 'SORVETE') columnToCheck = 'allow_sorvete'; 
        else { console.warn("Cargo não mapeado:", role); return; }
    ...
    const fetchData = async () => {
        try {
            ...
            if(mix) {
                setQuotaMixParams(mix.map(x => ({
                    ...x, 
                    storeId: x.store_id,
                    category_name: x.parent_category,
                    percentage: Number(x.mix_percentage || 0)
                })));
            }
    ...
    const handleLogin = async (email: string, pass: string, remember: boolean) => {
        try {
            ...
            const role = data.role_level.toUpperCase();
            await fetchPermissions(role);

            const loggedUser: User = { id: data.id, name: data.name, role: role as UserRole, email: data.email, storeId: data.store_id };
            setUser(loggedUser);
    ...
    // Encontra a loja do usuário atual para verificar configurações (ex: tem gelateria?)
    const currentUserStore = stores.find(s => s.id === user.storeId);
    const hasGelateria = currentUserStore ? currentUserStore.has_gelateria : false;
                ...
                <nav className="flex-1 space-y-6">
                    {[
                        ...
                        { title: 'Operacional', items: [ 
                            // Verifica permissão E se a loja tem gelateria ativa
                            { id: 'pdv_gelateria', label: 'PDV Gelateria', icon: IceCreamIcon, perm: 'MODULE_ICECREAM', requiredFeature: hasGelateria }, 
                        ...
                        // Filtra itens: 1. Se tem permissão do cargo. 2. Se a loja tem o recurso ativado (ex: gelateria)
            ...
            <div className="flex-1 flex flex-col h-full bg-[#f3f4f6] overflow-hidden text-blue-950">
                ...
                <main className="flex-1 overflow-y-auto relative no-scrollbar">
                    {(() => {
                        ...
                        if (currentView === 'metas' && can('MODULE_METAS')) return <GoalRegistration stores={stores} goalsData={goalsData} onSaveGoals={async (data) => { for(const row of data) { await supabase.from('monthly_goals').upsert({ store_id: row.storeId, year: row.year, month: row.month, revenue_target: row.revenue_target, pa_target: row.paTarget, pu_target: row.puTarget, ticket_target: row.ticketTarget, items_target: row.itemsTarget, business_days: row.businessDays, delinquency_target: row.delinquencyTarget, trend: row.trend }, { onConflict: 'store_id, year, month' }); } fetchData(); }} />;
                        // Fix: Error in file App.tsx on line 296: Property 'manager_percent' does not exist on type 'CotaSettings'. Did you mean 'managerPercent'?
                        ...
                        if (currentView === 'pdv_gelateria' && can('MODULE_ICECREAM')) return <IceCreamModule user={user!} stores={stores} items={iceCreamItems} sales={iceCreamSales} finances={iceCreamFinances} stock={iceCreamStock} promissories={icPromissories} can={can} onAddSales={async (s) => { 
                            // Fix: Corrected property access on IceCreamDailySale objects (s.map(x => ...)) to use camelCase (unitsSold, unitPrice, etc.)
                            await supabase.from('ice_cream_daily_sales').insert(s.map(x => ({ store_id: x.storeId, item_id: x.itemId, product_name: x.productName, category: x.category, flavor: x.flavor, units_sold: Math.round(x.unitsSold), unit_price: x.unitPrice, total_value: x.totalValue, payment_method: x.paymentMethod, sale_code: x.saleCode, buyer_name: x.buyer_name, status: 'active' }))); fetchData(); 
                        }} onCancelSale={async (code, reason) => { await supabase.from('ice_cream_daily_sales').update({ status: 'canceled', cancel_reason: reason }).eq('sale_code', code); await fetchData(); }} onUpdatePrice={async (id, p) => { await supabase.from('ice_cream_items').update({ price: p }).eq('id', id); await fetchData(); }} onAddTransaction={async (t) => { await supabase.from('ice_cream_finances').insert([{ store_id: t.storeId, date: t.date, type: t.type, category: t.category, value: t.value, description: t.description }]); await fetchData(); }} onAddItem={async (n, c, p, f, si, u, cps, tsId, r) => { await handleSaveIceCreamProduct({ storeId: tsId, name: n, category: c as any, price: p, flavor: f, recipe: r }); }} onSaveProduct={handleSaveIceCreamProduct} onDeleteItem={async (id) => { await supabase.from('ice_cream_items').delete().eq('id', id); await fetchData(); }} onUpdateStock={async (sId, b, v, u, type) => { const { data: current } = await supabase.from('ice_cream_stock').select('stock_current').eq('store_id', sId).eq('product_base', b).maybeSingle(); let finalVal = v; if (current && (type === 'purchase' || type === 'adjustment' || type === 'production')) { finalVal = Number(current.stock_current || 0) + v; } await supabase.from('ice_cream_stock').upsert({ store_id: sId, product_base: b, stock_current: finalVal, unit: u, is_active: true }, { onConflict: 'store_id, product_base' }); await fetchData(); }} liquidatePromissory={async (id) => { await supabase.from('ice_cream_promissory_notes').update({ status: 'paid' }).eq('id', id); await fetchData(); }} onDeleteStockItem={async (id) => { if (user?.role === UserRole.ADMIN) { await supabase.from('ice_cream_stock').update({ is_active: false }).eq('id', id); await fetchData(); } }} />;
                        
                        if (currentView === 'caixa' && can('MODULE_CASH_REGISTER')) return <CashRegisterModule user={user!} sales={iceCreamSales} finances={iceCreamFinances} closures={closures} receipts={receipts} errors={cashErrors} onAddClosure={async (c) => { await supabase.from('cash_register_closures').insert([{ store_id: user?.storeId, closed_by: user?.name, total_sales: c.totalSales, total_expenses: c.totalExpenses, balance: c.balance, notes: c.notes, date: c.date }]); fetchData(); }} onAddReceipt={async (r) => { await supabase.from('receipts').insert([{ store_id: r.storeId, issuer_name: r.issuer, payer: r.payer, recipient: r.recipient, value: r.value, value_in_words: r.valueInWords, reference: r.reference, date: r.date }]); fetchData(); }} onAddError={async (e) => { await supabase.from('cash_errors').insert([e]); fetchData(); }} onDeleteError={async (id) => { await supabase.from('cash_errors').delete().eq('id', id); fetchData(); }} />;
