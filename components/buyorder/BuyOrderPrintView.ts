...
export const printOrder = async (order: any, supabase: SupabaseClient) => {
  ...
  // 5. Grades Únicas — 1 template por letra, sem acumulação entre itens, priorizando a definição mais comum
  const printItems = loadedItems || [];
  const gradeOccurrences: Map<string, Map<string, { tamanhos: Record<string, number>, count: number }>> = new Map();

  for (const item of printItems) {
    if (!Array.isArray(item.grades)) continue;
  ...
  for (const [letra, variants] of gradeOccurrences) {
    ...
    // Filtrar tamanhos para manter apenas os com quantidade > 0
  ...
  gradesToShow.forEach(g => {
    Object.keys(g.tamanhos).forEach(sz => allSizesSet.add(sz));
  });

  const sortNum = (a: string, b: string) => (parseFloat(a) || 0) - (parseFloat(b) || 0);
  const sortedSizes = Array.from(allSizesSet).sort(sortNum);
  ...
  if (gradesToShow.length === 0) {
    ...
    gradesToShow.forEach(g => {
      ...
      gradesTableHtml += `
        <tr>
          <td class="grade-cell-letter"><b>${g.letra}</b></td>
          ${sortedSizes.map(sz => `<td>${tams[sz] !== undefined ? tams[sz] : ''}</td>`).join('')}
    ...
    loadedItems.forEach((item: any) => {
      ...
      if (match) {
        ...
        if (gradeObj && gradeObj.tamanhos) {
          const numLojasSub = Array.isArray(sub.lojas_numeros) ? sub.lojas_numeros.length : 1;
          const qtyPorLoja: number = Object.values(gradeObj.tamanhos as Record<string, any>).reduce((s: number, v: any) => s + Number(v || 0), 0);
          const qty = qtyPorLoja * numLojasSub;
          subPares += qty;
          if (isAcessorio(item)) {
            subUnidadesOnly += qty;
          } else {
  ...
  const formatMesAno = (dateStr: string): string => {
    const d = new Date(dateStr + 'T00:00:00');
    if (isNaN(d.getTime())) return '';
    return `${MESES_ABREV[d.getMonth()]}/${String(d.getFullYear()).slice(-2)}`;
  };

  const vencimentosDisplay = Array.isArray(order.vencimentos) && order.vencimentos.length > 0
    ? order.vencimentos.map(formatMesAno).filter(Boolean).join(' - ')
    : '';
...
<head>
  ...
  <style>
    ...
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .info-table th.col-narrow { width: 8%; }
    .info-table td.col-narrow { width: 10%; }
    .info-table td.col-wide { width: auto; }
...
<body>
  ...
  <table class="info-table">
    <tr>
      ...
      <th>LOJAS:</th>
      <td colspan="3">${lojasDisplay}</td>
    </tr>
    <tr>
      <th>REPR:</th>
      <td class="col-wide">${order.representante || '—'}${order.telefone_representante || order.telefone ? ` (${order.telefone_representante || order.telefone})` : ''}${(order.email_representante || order.email) ? ` · ${order.email_representante || order.email}` : ''}</td>
      <th>COMPRADOR:</th>
      <td>${order.user_name || '—'}</td>
      <th class="col-narrow">DESCONTO:</th>
      <td class="col-narrow">${desconto > 0 ? desconto.toFixed(1).replace('.', ',') + '%' : '—'}</td>
    </tr>
    <tr>
      <th>EMBARQUE:</th>
      <td>${getEmbarqueDisplay()}</td>
      <th>PRAZOS:</th>
      <td colspan="3">${order.prazos ? (Array.isArray(order.prazos) ? order.prazos.join(' / ') : order.prazos) : '—'} dias${vencimentosDisplay ? ` <span style="color:#888;font-size:9px;">(${vencimentosDisplay})</span>` : ''}</td>
    </tr>
  </table>
          ...
          <tr>
            ...
            ${loadedSubOrders.map(sub => {
              ...
              if (match) {
                ...
                if (gradeObj && gradeObj.tamanhos) {
                  const numLojasSub = Array.isArray(sub.lojas_numeros) ? sub.lojas_numeros.length : 1;
                  const qtyPorLoja = Object.values(gradeObj.tamanhos as Record<string, any>).reduce((s: number, v: any) => s + Number(v || 0), 0);
                  subParesQty = qtyPorLoja * numLojasSub;
                }
