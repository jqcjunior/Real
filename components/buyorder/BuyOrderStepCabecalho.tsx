...
function StepCabecalho({
  ...
  const representanteInputRef = useRef<HTMLInputElement>(null);
  ...
  const cabRef = useRef(cab);
  useEffect(() => {
    cabRef.current = cab;
  }, [cab]);

  const prazosRawRef = useRef(prazosRaw);
  useEffect(() => {
    prazosRawRef.current = prazosRaw;
  }, [prazosRaw]);
            ...
            <input
              ...
              onKeyDown={(e) => {
                if (e.key === "Enter") {
                  e.preventDefault();
                  setTimeout(() => {
                    const currentCab = cabRef.current;
                    const hasFornecedor = !!currentCab.fornecedor?.trim();
                    const hasRepresentante = !!currentCab.representante?.trim();
                    const hasTelefone = !!currentCab.telefone?.trim();
                    const hasEmail = !!currentCab.email?.trim();

                    if (hasFornecedor && hasRepresentante && hasTelefone && hasEmail) {
                      dataInicialRef.current?.focus();
                      dataInicialRef.current?.select?.();
                    } else {
                      fornecedorInputRef.current?.focus();
                      fornecedorInputRef.current?.select?.();
                    }
                  }, 120);
                }
              }}
            ...
            <input
              ...
              onKeyDown={(e) => {
                if (e.key === "Enter") {
                  e.preventDefault();
                  const currentCab = cabRef.current;
                  if (!currentCab.representante?.trim()) {
                    representanteInputRef.current?.focus();
                    representanteInputRef.current?.select?.();
                  } else if (!currentCab.telefone?.trim()) {
                    telefoneInputRef.current?.focus();
                    telefoneInputRef.current?.select?.();
                  } else if (!currentCab.email?.trim()) {
            ...
            <input
              ...
              onKeyDown={(e) => {
                if (e.key === "Enter") {
                  e.preventDefault();
                  const currentCab = cabRef.current;
                  const currentPrazosRaw = prazosRawRef.current;
                  if (currentCab.fat_fim && !currentPrazosRaw.trim()) {
