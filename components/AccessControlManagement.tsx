...
const AccessControlManagement: React.FC = () => {
    ...
    const togglePermission = async (perm: PagePermission, roleField: 'allow_admin' | 'allow_manager' | 'allow_cashier' | 'allow_sorvete' | 'allow_cobrador') => {
    ...
    const handleSaveNewPage = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setFormData({ page_key: '', label: '', module_group: 'Operação', allow_admin: true, allow_manager: false, allow_cashier: false, allow_sorvete: false, allow_cobrador: false, sort_order: 100 });
        } catch (err: any) {
                    ...
                    <table className="w-full text-left min-w-[1000px]">
                        <thead>
                            <tr className="bg-white text-[11px] font-black text-gray-400 uppercase tracking-widest border-b border-gray-100">
                                ...
                                <th className="px-6 py-6 text-center bg-amber-50/30">Cobrador</th>
                            ...
                            {isLoading ? (
                                ...
                                <tr key={perm.id} className="hover:bg-gray-50 transition-all group">
                                    ...
                                    <td className="px-6 py-6 text-center bg-amber-50/10">
                                        <button 
                                            onClick={() => togglePermission(perm, 'allow_cobrador')} 
                                            className={`w-12 h-12 rounded-2xl transition-all active:scale-90 flex items-center justify-center border-2 ${perm.allow_cobrador ? 'bg-amber-600 text-white border-amber-500 shadow-xl shadow-amber-100' : 'bg-white text-gray-200 border-gray-100 hover:border-amber-200 hover:text-amber-200'}`}
                                        >
                                            {perm.allow_cobrador ? <Check size={24} strokeWidth={4} /> : <X size={20} />}
                                        </button>
                                    </td>
