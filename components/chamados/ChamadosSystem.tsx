...
import {
    ...
    PanelRightOpen, PanelRightClose, Users2, BarChart3, X
} from 'lucide-react';
...
const ChamadosSystem: React.FC<ChamadosSystemProps> = ({ user, stores, onUnreadUpdate }) => {
    ...
    // @ts-ignore
    const currentUserRole = String(user.role || '').toUpperCase().trim();
    const isAdmin = currentUserRole === 'ADMIN';

    const [selectedStoreId, setSelectedStoreId] = useState<string | null>(isAdmin ? null : user.storeId || user.store_id || null);
    ...
    const filteredStores = useMemo(() => {
        ...
        if (!isAdmin) return stores.filter(s => s.id === userStoreId);
        return stores
    ...
    useEffect(() => {
        if (selectedDemand) {
            ...
            if (isAdmin) {
                supabase.rpc('fn_mark_demand_notifications_seen', {
                    p_demand_id: selectedDemand.id
                }).then(({ error }) => {
                    if (error) {
                        console.error('Erro ao marcar notificações do chamado como lidas por RPC:', error);
                    }
                });
            }
        }
    ...
    return (
        <div className="flex flex-col h-full min-h-[600px] bg-transparent relative">
            {/* Header */}
            <div className="bg-white dark:bg-slate-900 p-2 lg:py-2.5 lg:px-4 border-b border-slate-200 dark:border-slate-800 flex flex-col sm:flex-row justify-between items-center gap-2 lg:gap-3">
                <div className="flex items-center gap-2 lg:gap-3 w-full lg:w-auto">
                    <div className="p-1.5 lg:p-2 bg-blue-600 rounded-lg lg:rounded-xl shadow-md shadow-blue-900/10">
                        <MessageSquare className="text-white" size={16} />
                    </div>
                    <div className="flex-1 lg:flex-none">
                        <h2 className="text-xs lg:text-sm font-black text-slate-900 dark:text-white uppercase italic tracking-tighter">
                            Central de Chamados <span className="text-blue-600">V3.0</span>
                        </h2>
                        <p className="text-[8px] font-bold text-slate-400 uppercase tracking-widest hidden xl:block">
                ...
                <div className="flex items-center gap-1.5 lg:gap-2 shrink-0">
                    {mobileView === 'chat' && (
                        <button onClick={() => { setMobileView('list'); setSelectedDemand(null); }} className="lg:hidden p-1.5 bg-slate-100 dark:bg-slate-800 text-slate-600 rounded-lg active:scale-95 transition-all">
                    ...
                    {selectedDemand && (
                        <button
                            onClick={() => {
                                if (showSidePanel && sidePanelTab === 'stats') {
                            ...
                            className={`p-1.5 lg:p-2 rounded-lg transition-all active:scale-95 flex items-center justify-center ${
                                showSidePanel && sidePanelTab === 'stats'
                                    ? 'bg-blue-600 text-white shadow-md'
                                    : 'bg-slate-100 dark:bg-slate-800 text-slate-500 hover:text-blue-600 hover:bg-blue-50 dark:hover:bg-slate-700/50'
                            }`}
                            title="Estatísticas"
                        >
                            <BarChart3 size={16} />
                    ...
                    <button onClick={() => setShowNewDemandModal(true)} className="p-1.5 lg:p-2 bg-blue-600 text-white rounded-lg shadow-md hover:bg-blue-700 transition-all active:scale-95">
                        <Plus size={16} />
                    </button>
                </div>
            </div>
                    ...
                    <div className="flex-1 flex flex-col overflow-hidden bg-white dark:bg-slate-900">
                                ...
                                <div className="flex-1 min-w-0">
                                    <div className="flex items-center gap-2 mb-2 flex-wrap">
                                        ...
                                        <span className="text-[10px] font-medium text-slate-400">
                                            Criado {formatDistanceToNow(new Date(selectedDemand.created_at), { addSuffix: true, locale: ptBR })}
                                        </span>
                        ...
                        <div ref={messagesScrollRef} className="flex-1 overflow-y-auto p-4 space-y-4 bg-slate-50 dark:bg-slate-950">
                            ...
                            {messages.map((msg, index) => {
                                const showSeparator = index === 0 || isDifferentDay(msg.created_at, messages[index - 1].created_at);
                                return (
                                    <React.Fragment key={msg.id}>
                                        {showSeparator && (
                                            <div className="flex justify-center my-2 animate-in fade-in duration-300">
                                                <span className="px-3 py-1 bg-slate-100 dark:bg-slate-800 rounded-full text-[9px] font-black text-slate-400 uppercase">
                                                    {getDateLabel(msg.created_at)}
                                                </span>
                                            </div>
                                        )}
                                        {/* @ts-ignore */}
                                        <ChamadoMessageBubble msg={msg} isMe={msg.sender_id === currentUserId} />
                                    </React.Fragment>
                                );
                            })}
            ...
            <div className="flex-1 hidden lg:flex overflow-hidden relative">
                                    ...
                                    <div className="flex items-center gap-2 shrink-0">
                                        {isAdmin && (
                                            <>
                                                ...
                                                <button
                                                    onClick={() => {
                                                        if (showSidePanel && sidePanelTab === 'assign') {
                                                            setShowSidePanel(false);
                                                        } else {
                                                            setSidePanelTab('assign');
                                                            setShowSidePanel(true);
                                                        }
                                                    }}
                                                    className={`p-2 rounded-xl transition-all active:scale-95 flex items-center justify-center ${
                                                        showSidePanel && sidePanelTab === 'assign'
                                                            ? 'bg-blue-600 text-white shadow-md'
                                                            : 'bg-slate-100 dark:bg-slate-800 text-slate-500 hover:text-blue-600 hover:bg-blue-50 dark:hover:bg-slate-700/50'
                                                    }`}
                                                    title="Direcionar Chamado"
                                                >
                                                    <Users2 size={18} />
                                                </button>
                                            </>
                ...
                {/* Painel DESLIZANTE — sob demanda, acionado pelos botões de estatísticas e direcionamento */}
                {showSidePanel && selectedDemand && (
                    <div className="absolute top-0 right-0 bottom-0 w-[320px] max-w-[85vw] bg-white dark:bg-slate-900 border-l border-slate-200 dark:border-slate-800 shadow-2xl z-20 flex flex-col animate-in slide-in-from-right duration-200">
                        <div className="flex border-b border-slate-200 dark:border-slate-800 shrink-0 items-center pr-2">
                            ...
                            <button
                                ...
                                className="p-1.5 text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 rounded-lg transition-all hover:bg-slate-100 dark:hover:bg-slate-800"
                                title="Fechar"
                            >
                                <X size={16} />
                            </button>
            ...
            <NovoChamadoModal
                ...
                stores={stores} userStoreId={user.storeId || user.store_id || null} isSubmitting={isLoading} onSubmit={handleCreateDemand}
            />
        </div>
    );
};
