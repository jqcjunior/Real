        ...
        <div className="bg-white dark:bg-slate-900 p-4 sm:p-5 md:p-6 rounded-2xl sm:rounded-3xl shadow-sm border border-slate-100 dark:border-slate-800 flex flex-col gap-4">
            <div className="flex items-center justify-between gap-3 sm:gap-4">
                ...
                <button 
                    ...
                    className="px-3 py-2 bg-slate-900 dark:bg-slate-800 hover:bg-slate-800 dark:hover:bg-slate-700 text-white rounded-xl shadow-lg active:scale-95 transition-all flex items-center gap-1.5 text-[9px] sm:text-[10px] font-black uppercase tracking-wider border border-slate-750 shrink-0"
                >
                    {isLoadingAi ? (
                    ...
                    <span>{isLoadingAi ? 'Analisando...' : 'IA Insight'}</span>
                </button>
            </div>
            
            {/* Filtros Ativos / Seleção de Período */}
            <div className="flex flex-col gap-3 pt-3 border-t border-slate-100 dark:border-slate-800">
                <div className="flex flex-col gap-1.5">
                    <span className="text-[8px] sm:text-[9px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest flex items-center gap-1">
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                        {(() => {
                            ...
                            const activeMonth = selectedMonth.split('-')[1];
                            return years.map(y => {
                                const isActive = String(y) === activeYear;
                                return (
                                    <button
                                        key={y}
                                        onClick={() => setSelectedMonth(`${y}-${activeMonth}`)}
                                        className={`px-3 py-1.5 rounded-xl text-[9px] sm:text-[10px] font-black uppercase tracking-wider transition-all border ${
                                            isActive 
                ...
                <div className="flex flex-col gap-1.5 mt-1">
                    <span className="text-[8px] sm:text-[9px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest flex items-center gap-1">
                        <Percent size={10} className="text-blue-600 dark:text-blue-400" />
                        Mês de Referência
                    </span>
                    {/* Month Pills Container */}
                    <div className="flex items-center gap-1.5 overflow-x-auto pb-1.5 -mx-4 px-4 scrollbar-thin scrollbar-thumb-slate-200 dark:scrollbar-thumb-slate-800">
                        {(() => {
                            const months = [
                                { value: '01', label: 'Jan' },
                                { value: '02', label: 'Fev' },
                            ...
                            const [activeYear, activeMonth] = selectedMonth.split('-');
                            return months.map(m => {
                                const isActive = m.value === activeMonth;
                                return (
                                    <button
                                        key={m.value}
                                        onClick={() => setSelectedMonth(`${activeYear}-${m.value}`)}
                                        className={`px-3 py-1.5 rounded-xl text-[9px] sm:text-[10px] font-black uppercase tracking-wider transition-all border shrink-0 ${
                                            isActive 
                                                ? 'bg-slate-900 border-slate-900 dark:bg-white dark:border-white text-white dark:text-slate-950 shadow-md shadow-slate-950/20 scale-105' 
                                                : 'bg-slate-50 hover:bg-slate-100 dark:bg-slate-800 dark:hover:bg-slate-700 border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300'
                                        }`}
                                    >
                                        {m.label}
