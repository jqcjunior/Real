...
import { TrendingUp, Target, ShoppingBag, CreditCard, Tag, Upload, FileSpreadsheet, Loader2, CheckCircle, X, Save, Package, Trophy, Medal, Star, Crown, DollarSign, ArrowUpRight, ArrowDownRight, BrainCircuit, Sparkles, Zap, Box, Percent, Hash } from 'lucide-react';
    ...
    <div className="p-8 max-w-full mx-auto space-y-8 animate-in fade-in duration-700">
       ...
       <div className="flex flex-col xl:flex-row justify-between items-start xl:items-center gap-6">
          <div>
            ...
            <p className="text-gray-500 font-bold uppercase text-[10px] tracking-[0.3em] mt-3 ml-1">Command Center | Real Calçados v4.0</p>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-3">
             ...
             <button onClick={() => window.dispatchEvent(new CustomEvent('changeView', { detail: 'metas_registration' }))} className="flex items-center gap-2 bg-gray-900 text-white px-6 py-3 rounded-2xl hover:bg-black transition-all shadow-xl font-black uppercase text-[10px] tracking-widest border-b-4 border-red-600 active:translate-y-1">
                <Target size={16} /> Definir Metas
             </button>
             <button onClick={() => setShowImportModal(true)} className="flex items-center gap-2 bg-green-600 text-white px-6 py-3 rounded-2xl hover:bg-green-700 transition-all shadow-xl font-black uppercase text-[10px] tracking-widest border-b-4 border-green-800 active:translate-y-1">
       ...
       <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
           ...
           <div className="bg-white p-8 rounded-[40px] shadow-sm border border-gray-100 relative overflow-hidden group">
               ...
               <p className="text-[9px] font-bold text-gray-400 mt-4 uppercase italic">Objetivo consolidado do mês</p>
           </div>

           <div className="bg-gradient-to-br from-blue-900 to-blue-950 p-8 rounded-[40px] shadow-2xl text-white relative group">
               <div className="absolute top-0 right-0 p-6 opacity-10"><Zap size={80}/></div>
               <p className="text-[10px] font-black text-blue-300 uppercase tracking-widest mb-2">Lojas Ativas</p>
               <h3 className="text-4xl font-black italic tracking-tighter">{rankedStores.length} <span className="text-sm not-italic font-bold text-blue-400">UNIDADES</span></h3>
               <div className="mt-4 flex items-center gap-2 text-green-400">
                   <ArrowUpRight size={14}/>
                   <span className="text-[10px] font-black uppercase tracking-wider">Monitoramento em Tempo Real</span>
               </div>
           </div>
           ...
           <div className="bg-white p-4 rounded-[40px] shadow-sm border border-gray-100 flex flex-col justify-center items-center text-center group cursor-pointer hover:border-blue-200 transition-all" onClick={handleGenerateNetworkInsight}>
               {isLoadingAi ? (
                   <div className="flex flex-col items-center gap-2">
                       ...
                       <p className="text-[10px] font-black text-gray-400 uppercase animate-pulse">Analisando Rede...</p>
                   </div>
               ) : (
                   <>
                       ...
                       <p className="text-[8px] text-gray-400 font-bold uppercase mt-1">Gerar Insight Estratégico</p>
                   </>
               )}
           </div>
       </div>
       ...
       {aiInsight && (
           <div className="bg-white border-2 border-blue-100 rounded-[40px] p-8 shadow-xl animate-in slide-in-from-top-4 duration-500">
                <div className="flex items-center gap-3 mb-4">
                    <div className="w-10 h-10 bg-blue-600 rounded-2xl flex items-center justify-center text-white shadow-lg shadow-blue-200"><Sparkles size={20}/></div>
       ...
       <div className="bg-white rounded-[48px] shadow-xl border border-gray-100 overflow-hidden">
          <div className="p-8 border-b border-gray-50 flex justify-between items-center bg-gray-50/50">
             <div>
                ...
                <p className="text-[9px] text-gray-400 font-bold uppercase tracking-widest mt-1">Visualização Completa dos Indicadores de Venda</p>
             </div>
          </div>
                        ...
                        <tr key={item.storeId} className="hover:bg-blue-50/40 transition-all group">
                            ...
                            <td className="px-6 py-6">
                                <div className="flex flex-col">
                                    ...
                                    <span className="text-[8px] font-bold text-gray-400 uppercase tracking-widest mt-1">Real Calçados</span>
                                </div>
                            </td>

                            {/* Faturamento */}
                            <td className="px-6 py-6 text-right">
                                <span className="text-sm font-black text-gray-900">{formatCurrency(item.revenueActual)}</span>
                            </td>

                            {/* XP % */}
                            <td className="px-6 py-6">
                                <div className="flex flex-col items-center gap-2">
                                    <span className={`text-xs font-black italic ${item.xp >= 100 ? 'text-green-600' : 'text-blue-700'}`}>{item.xp.toFixed(1)}%</span>
                                    <div className="w-24 h-1.5 bg-gray-100 rounded-full overflow-hidden shadow-inner border border-gray-50">
                                        <div 
                                            className={`h-full transition-all duration-1000 ${item.xp >= 100 ? 'bg-green-500' : 'bg-blue-600'}`} 
                                            style={{ width: `${Math.min(item.xp, 100)}%` }}
                                        />
                                    </div>
                                </div>
                            </td>
                            ...
                            <td className="px-6 py-6 text-center">
                                {item.xp >= 100 ? (
                                    <span className="px-3 py-1 bg-green-50 text-green-700 rounded-full text-[8px] font-black uppercase border border-green-200">Meta OK</span>
                                ) : (
              ...
              <div className="bg-white rounded-[48px] p-10 max-w-md w-full shadow-2xl animate-in zoom-in duration-300 border-t-8 border-green-600">
                  <div className="flex justify-between items-center mb-8">
                      <div>
                        ...
                        <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mt-1">Alimentar indicadores da rede</p>
                      </div>
                  ...
                  <div 
                    ...
                    className={`border-4 border-dashed rounded-[32px] p-12 flex flex-col items-center justify-center mb-8 cursor-pointer transition-all ${selectedFile ? 'border-green-400 bg-green-50 shadow-inner' : 'border-gray-100 hover:bg-gray-50 hover:border-blue-400'}`}
                  >
                      {selectedFile ? (
                          <>
                             <CheckCircle size={48} className="text-green-500 mb-4" />
                             <span className="text-xs font-black text-green-700 uppercase italic text-center truncate w-full px-4">{selectedFile.name}</span>
                          </>
                      ) : (
                          <>
                             <FileSpreadsheet size={48} className="text-gray-200 mb-4" />
                             <span className="text-[10px] text-gray-400 font-black uppercase tracking-widest text-center">Selecionar Planilha Corporativa</span>
                          </>
                      )}
                  </div>
                  ...
                  <div className="flex gap-4">
                      <button onClick={() => setShowImportModal(false)} className="flex-1 py-5 bg-gray-50 text-gray-400 rounded-3xl font-black uppercase text-[10px] tracking-widest hover:bg-gray-100 transition-all">Voltar</button>
                      <button onClick={handleProcessImport} disabled={!selectedFile || isProcessing} className="flex-1 py-5 bg-blue-700 text-white rounded-3xl font-black uppercase text-[10px] tracking-widest shadow-xl shadow-blue-200 hover:bg-blue-800 disabled:opacity-50 transition-all border-b-4 border-blue-900 active:translate-y-1">Efetivar</button>
                  </div>
              </div>
          </div>
       )}
