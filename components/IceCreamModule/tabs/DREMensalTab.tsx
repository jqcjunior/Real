...
const DREMensalTab: React.FC<DREMensalTabProps> = ({
    ...
    return (
        <div className="p-4 md:p-8 space-y-6 animate-in fade-in duration-300 max-w-7xl mx-auto pb-20">
            {/* CABEÇALHO */}
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h3 className="text-2xl font-black uppercase italic text-blue-950 tracking-tighter">
                            DRE <span className="text-purple-700">Mensal</span>
                        </h3>
                ...
                <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 w-full md:w-auto">
                    <div className="flex items-center gap-2 bg-gray-50 p-2 rounded-2xl border">
                        <select 
                            ...
                            className="bg-transparent border-none text-xs font-black uppercase text-slate-900 outline-none"
                        >
                        ...
                        <select 
                            ...
                            className="bg-transparent border-none text-xs font-black text-slate-900 outline-none"
                        >
                    ...
                    <div className="flex gap-2 flex-wrap sm:flex-nowrap">
                        <button 
                            onClick={handlePrintDreMensal} 
                            className="flex-1 sm:flex-none px-4 py-2.5 bg-gray-900 text-white rounded-xl font-black uppercase text-[9px] shadow-lg flex items-center justify-center gap-2 border-b-4 border-gray-700 active:scale-95"
                        >
                        ...
                        <button 
                            ...
                            className="flex-1 sm:flex-none px-4 py-2.5 bg-red-600 text-white rounded-xl font-black uppercase text-[9px] shadow-lg flex items-center justify-center gap-2 border-b-4 border-red-800 active:scale-95"
                        >
                            <Printer size={14}/> Despesas
                        </button>
                    </div>
                </div>
            </div>
 
            {/* GRID RESPONSIVO - DEMONSTRATIVO + PARTILHA LADO A LADO */}
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                {/* DEMONSTRATIVO - 8 COLUNAS */}
                <div className="lg:col-span-8 bg-white p-6 md:p-8 rounded-[40px] shadow-sm border border-gray-100">
                    <h4 className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] mb-6 flex items-center gap-2">
                        <Briefcase size={14}/> Demonstrativo de Resultado
                    </h4>
                    <div className="space-y-4">
                        <div className="flex justify-between items-center pb-4 border-b">
                            <span className="font-bold text-gray-600 uppercase text-xs">Faturamento (+)</span>
                            <span className="font-black text-green-600 text-lg md:text-xl">{formatCurrency(dreStats.monthIn)}</span>
                        </div>
                        <div 
                            className="flex justify-between items-center pb-4 border-b cursor-pointer hover:bg-red-50 transition-all rounded-lg px-2 -mx-2" 
                            onClick={() => setShowSangriaDetailModal('month')}
                        >
                            <span className="font-bold text-gray-600 uppercase text-xs flex items-center gap-2">
                                Despesas / Saídas (-) <ExternalLink size={10}/>
                            </span>
                            <span className="font-black text-red-600 text-lg md:text-xl">{formatCurrency(dreStats.monthSangriaTotal)}</span>
                        </div>
                        <div className="flex justify-between items-center pt-2">
                            <span className="font-black text-blue-950 uppercase text-sm">Lucro Líquido (=)</span>
                            <span className="font-black text-blue-900 text-2xl md:text-3xl italic">{formatCurrency(dreStats.profit)}</span>
                        </div>
                    </div>
                </div>
 
                {/* PARTILHA DE LUCROS - 4 COLUNAS */}
                <div className="lg:col-span-4 bg-gray-950 p-6 md:p-8 rounded-[40px] text-white shadow-2xl flex flex-col">
                    <div className="flex justify-between items-center mb-6">
                        <h4 className="text-[10px] font-black text-blue-400 uppercase tracking-[0.2em] flex items-center gap-3">
                            <UsersIcon size={16}/> Partilha de Lucros
                        </h4>
                        {can('admin_settings') && (
                    ...
                    <div className="space-y-4 flex-1">
                        {partners.map(p => (
                            <div key={p.id} className="border-b border-white/5 pb-4 last:border-0">
                                ...
                                <p className="text-xl font-black italic tracking-tighter text-blue-100">
                                    {formatCurrency((dreStats.profit * p.percentage) / 100)}
                                </p>
                            </div>
                        ))}
                        {partners.length === 0 && (
                            <p className="text-[9px] text-gray-600 uppercase text-center py-10 italic">
                                Nenhuma partilha configurada
                            </p>
                        )}
                    </div>
                </div>
            ...
            {/* RESUMO OPERACIONAL - LARGURA COMPLETA */}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
                    ...
                    <div className="p-4 bg-blue-950 rounded-2xl border border-blue-900 text-white">
                        ...
                        <p className="text-xl font-black italic">
                            {dreStats.monthIn > 0 ? ((dreStats.profit / dreStats.monthIn) * 100).toFixed(1) : '0.0'}%
                        </p>
                    </div>
                </div>
            </div>
 
            {/* DETALHAMENTO DE VENDAS - LARGURA COMPLETA */}
            <div className="bg-white p-6 md:p-8 rounded-[40px] shadow-sm border border-gray-100">
                <h4 className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] mb-6 flex items-center gap-2">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                    {dreStats.monthSalesDetail.sort((a: any, b: any) => b.totalValue - a.totalValue).map((item: any) => (
                        <div key={item.productName} className="flex justify-between items-center bg-gray-50 p-4 rounded-2xl border border-gray-100 hover:shadow-md transition-all">
                            <div className="flex flex-col min-w-0 flex-1">
                                <span className="text-[10px] font-black uppercase text-blue-950 truncate">{item.productName}</span>
                                <span className="text-purple-600 font-black uppercase mt-1">
                                    QTD: <span className="text-lg font-black">{item.quantity}</span>
                                </span>
                            </div>
                            <span className="text-xs font-black text-gray-900 ml-2">{formatCurrency(item.totalValue)}</span>
                        </div>
                    ))}
                    {dreStats.monthSalesDetail.length === 0 && (
                        <p className="col-span-full text-center text-[9px] text-gray-400 uppercase italic py-4">
            ...
            {/* MÉTODOS DE PAGAMENTO - LARGURA COMPLETA */}
            <div className="bg-white p-6 md:p-8 rounded-[40px] shadow-sm border border-gray-100">
            ...
            {/* DÉBITO FUNCIONÁRIOS - LARGURA COMPLETA */}
            <div className="bg-white rounded-[40px] shadow-xl border border-gray-100 overflow-hidden">
                <div className="p-6 bg-red-50 border-b border-red-100 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                    <div>
                        <h4 className="text-xs font-black uppercase text-red-700 flex items-center gap-2">
                            <ClipboardList size={16}/> Débito Funcionário - Resumo Mensal
                        </h4>
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-left">
                        <thead className="bg-gray-50 text-[9px] font-black text-gray-400 uppercase tracking-widest border-b">
                            <tr>
                                <th className="px-4 sm:px-8 py-4">Colaborador / Profissional</th>
                                <th className="px-4 sm:px-8 py-4 text-right">Total Acumulado (R$)</th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-gray-50 font-bold text-[10px]">
                            {monthFiadoGrouped.map((f, i) => (
                                <React.Fragment key={f.name}>
                                    ...
                                    >
                                        <td className="px-4 sm:px-8 py-4 flex items-center gap-3">
                                            {expandedEmployee === f.name ? 
                                                <ChevronUp size={14} className="text-red-500 shrink-0" /> : 
                                                <ChevronDown size={14} className="text-gray-400 shrink-0" />
                                            }
                                            <span className="text-blue-950 uppercase italic text-xs tracking-tighter truncate">
                                                {f.name}
                                            </span>
                                        </td>
                                        <td className="px-4 sm:px-8 py-4 text-right text-red-700 text-sm font-black italic">
                                            {formatCurrency(f.total)}
                                        </td>
                                    </tr>
                                    {expandedEmployee === f.name && (
                                        <tr className="bg-gray-50/50">
                                            <td colSpan={2} className="px-4 sm:px-12 py-6">
                                                <div className="bg-white rounded-2xl border border-red-100 shadow-sm overflow-hidden">
                                                    <div className="p-3 bg-red-600 text-white text-[8px] font-black uppercase tracking-widest">
                                                        Histórico Detalhado: {f.name}
                                                    </div>
                                                    <div className="overflow-x-auto">
                                                        <table className="w-full text-[9px] font-bold">
                                                            <thead className="bg-gray-50 border-b border-gray-100 text-gray-400 uppercase">
                                                                <tr>
                                                                    <th className="px-4 py-2 text-gray-900">Data</th>
                                                                    <th className="px-4 py-2 text-gray-900">Cód. Venda</th>
                                                                    <th className="px-4 py-2 text-gray-900">Item</th>
                                                                    <th className="px-4 py-2 text-right text-gray-900">Valor</th>
                                                                </tr>
                                                            </thead>
                                                            <tbody className="divide-y divide-gray-50">
                                                                {f.items.map((item: any, idx: number) => (
                                                                    <tr key={idx}>
                                                                        ...
                                                                        <td className="px-4 py-2 uppercase italic text-blue-950 font-black">
                                                                            {item.productName}
                                                                        </td>
                                                                        <td className="px-4 py-2 text-right text-red-600 font-black">
                            ...
                            {monthFiadoGrouped.length === 0 && (
                                <tr>
                                    <td colSpan={2} className="p-16 text-center text-gray-400 uppercase tracking-[0.3em] italic">
                                        Nenhuma compra no fiado
                                    </td>
                                </tr>
                            )}
                        </tbody>
            ...
            {/* VENDAS CANCELADAS - LARGURA COMPLETA */}
            <div className="bg-white rounded-[40px] shadow-xl border border-gray-100 overflow-hidden">
                <div className="p-6 bg-gray-50 border-b border-gray-100 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                    <div>
                        <h4 className="text-xs font-black uppercase text-gray-700 flex items-center gap-2">
                            <XCircle size={16} className="text-gray-400" /> Vendas Canceladas - Detalhamento Mensal
                        </h4>
                        <p className="text-[9px] font-bold text-gray-400 uppercase mt-1">
                            Histórico de cancelamentos do período
                        </p>
                    </div>
                    <button 
                            ...
                            <tbody className="divide-y divide-gray-50 font-bold text-[10px]">
                                {dreStats.monthCanceledDetails.map((c: any) => (
                                    <tr key={c.id} className="hover:bg-gray-50/50 transition-all">
                                        ...
                                        <td className="px-4 sm:px-8 py-4 text-right text-gray-900 font-black italic">
